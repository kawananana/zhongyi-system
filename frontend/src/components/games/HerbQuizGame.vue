...
import { buildQuizQuestions, loadHerbPool, type QuizQuestion } from '@/utils/herbGamePool'
...
async function load() {
  loading.value = true
  try {
    const herbs = await loadHerbPool(50)
    questions.value = buildQuizQuestions(herbs, 8)
...
function next() {
  if (index.value >= questions.value.length - 1) {
    ...
    scoreStore.addPoints(sessionPoints.value, 'quiz', 'herb-quiz', stars)
    return
  }
  index.value += 1
  selected.value = null
...
<template>
  <GameShell title="本草问答" subtitle="根据性味归经、功效主治答题闯关" @back="emit('back')">
    ...
    <div v-else-if="current" class="quiz-panel">
      ...
      <div v-if="answered" class="feedback">
        ...
        <p v-else class="fail">
          ...
          <span v-if="current.herbName">（{{ current.herbName }}）</span>
        </p>
